...
lazy_static::lazy_static!(
    static ref DATA: Vec<HashMap<String, String>> = {
        ...
        let data: String = std::fs::read_to_string("default_data.json").expect("Failed to read data.json");

        // Parse the data.json file
        return serde_json::from_str(&data).expect("Failed to parse default_data.json");
    };
);
